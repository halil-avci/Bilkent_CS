...
public class GameManager 
{
    ...
    public void setupGame() 
    {
        ...
        bug.positionAt( TARGET_X / 2, TARGET_Y / 2);
    ...
    {
        //Initialize all cases of being together
        boolean PB = false;
        boolean MP = false;
        boolean MB = false;
        boolean MPB = false;
        ...
        for (int row = 0; row < NO_OF_ROWS; row++) {
            ...
            for (int column = 0; column < NO_OF_COLUMNS; column++) {
                ...
                else if (doCoordsMatch( player.getX(), player.getY(), monster.getX(), monster.getY())) {
                    MP = true;
                }
                else if (doCoordsMatch( monster.getX(), monster.getY(), bug.getX(), bug.getY())) {
                    MB = true;
                }

                if (doCoordsMatch( player.getX(), player.getY(), column, row)) {
                    output = " " + player.getIcon() + " |";

                    if (MPB) {
                        output = monster.getIcon() + player.getIcon() + bug.getIcon() + "|";
                    }
                    else if (PB) {
                        output = " " + player.getIcon() + bug.getIcon() + "|";
                    }
                    else if (MP) {
                        output = "" + monster.getIcon() + player.getIcon() + " |";
                    }
                }
                else if (doCoordsMatch( bug.getX(), bug.getY(), column, row)) {
                    output = " " + bug.getIcon() + " |";

                    if (MB) {
                        output = monster.getIcon() + " " + bug.getIcon() + "|";
                    }
                }
                else if (doCoordsMatch( monster.getX(), monster.getY(), column, row)) {
                    output = " " + monster.getIcon() + " |";
                }
    ...
    {
        if (doCoordsMatch( player.getX(), player.getY(), bug.getX(), bug.getY())) {
            player.loseHealth(BUG_DAMAGE_TO_PLAYER);
        }
        if (doCoordsMatch( player.getX(), player.getY(), monster.getX(), monster.getY())) {
            player.loseHealth(MONSTER_DAMAGE_TO_PLAYER);
        }
    }
    ...
    public char readDirection()
    {
        ...
        System.out.print( "\nw: up, x: down, d: right, a: left, s: no change, q: quit: ");
    ...
    {
        if (player.getY() == TARGET_Y && player.getX() == TARGET_X) {
            return true;
        }
        return false;
    }
